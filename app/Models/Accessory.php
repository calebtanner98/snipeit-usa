...
class Accessory extends SnipeModel
{
    ...
    */
    ...
    // Added By USA Team For Request Accessory Functionality
    ...
    {
        return $this->hasMany(CheckoutRequest::class, 'accessory_id')->whereNull('canceled_at')->whereNull('fulfilled_at')
        ->whereNull('checkedin_at');
    }
    
    public function cancelRequest($userId, $accessory_id, $cancelReason = null)
    {
        $checkoutRequest = $this->requests()->where('user_id', $userId)->where('accessory_id', $accessory_id)->first();
