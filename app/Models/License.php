...
class License extends Depreciable
{
    ...
    {
        return $query->leftJoin('users as users_sort', 'licenses.user_id', '=', 'users_sort.id')
        ->select('licenses.*')
        ->orderBy('users_sort.first_name', $order)
        ->orderBy('users_sort.last_name', $order);
    }

    // Added By USA Team For Request License Functionality
    ...
    {
        return $this->hasMany(CheckoutRequest::class, 'license_id')->whereNull('canceled_at')->whereNull('fulfilled_at')->whereNull('checkedin_at');
    }
    
    public function cancelRequest($userId, $licenseId, $cancelReason = null)
    {
        $checkoutRequest = $this->requests()->where('user_id', $userId)->where('license_id', $licenseId)->first();
    ...
    // End Request License Functionality
}