...
class AccessoriesTransformer
{
    ...
    public function transformRequestedAccessory($accessories, int $total)
    {
        return [
            'total' => $total,
            'accessories' => $accessories->map(function ($accessory) {
                return $this->transformSingleAccessory($accessory);
            }),
        ];
    }
    
    protected function transformSingleAccessory(Accessory $accessory)
    {
        $user = auth()->user();
        $waitlist = new WaitList();
        $isOnWaitList = $waitlist->isOnAccessoryWaitList($user, $accessory->id);
        $assignedToUser = AccessoryCheckout::where('assigned_to', $user->id)->where('accessory_id', $accessory->id)->exists();
        $assignmentRecord = AccessoryCheckout::where('assigned_to', $user->id)->where('accessory_id', $accessory->id)->exists();

        return [
            'id' => $accessory->id,
            'name' => e($accessory->name ?? null),
            'manufacturer_name'=> e($accessory->manufacturer->name ?? null),
            'model_number' => e($accessory->model_number ?? null),
            'supplier_name' => e($accessory->supplier->name ?? null),
            'location' => e($accessory->location->name ?? null),
            'qty' => e($accessory->qty ?? null),
            'is_requested_by_user' => e($accessory->isRequestedBy($user)),
            'is_on_waitlist' => $isOnWaitList,
            'is_assigned_to_user' => $assignedToUser,
            'checkedin_at' => $assignmentRecord,
