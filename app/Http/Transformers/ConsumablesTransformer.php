...
class ConsumablesTransformer
{
    ...
    protected function transformSingleConsumable(Consumable $consumable)
    {
        $user = auth()->user();
        $waitlist = new WaitList();
        $isOnWaitList = $waitlist->isOnConsumableWaitList($user, $consumable->id);

        return [
            'id' => $consumable->id,
            'name' => $consumable->name,
            'category' => $consumable->category->name,
            'qty' => $consumable->numRemaining(),
            'is_requested_by_user' => $consumable->isRequestedBy($user),
     ...
     */
    public function transformRequestedConsumables($consumables, $total)
    {
        return [
            'total' => $total,
            'consumables' => $consumables->map(function ($consumable) {
                return $this->transformSingleConsumable($consumable);
            })->toArray(),
        ];
    }
}