...
class AssetsTransformer
{
    ...
    public function transformRequestedAssets(Collection $assets, $total)
    {
        ...
        // Sort the array based on the status priority
        usort($array, function ($a, $b) {
            return $this->getAssetPriority($a) <=> $this->getAssetPriority($b);
        });
    
        return (new DatatablesTransformer)->transformDatatables($array, $total);
    }
    
    private function getAssetPriority(array $asset)
    {
        // Priority mapping

        if (!$asset['requested_by_user'] && !$asset['assigned_to'] && !$asset['is_on_waitlist']) {
            return 1;
        }
        if ($asset['requested_by_user']) {
            return 2; 
        }
        if (!$asset['requested_by_user'] && !$asset['assigned_to'] && !$asset['is_on_waitlist']) {
            return 3; 
        }
        if ($asset['is_on_waitlist']) {
            return 4;
        }
    ...
    public function transformRequestedAsset(Asset $asset)
    {
        ...
        $isOnWaitList = $waitlist->isOnAssetWaitList($user, $asset->id);
        ...
        $array = [
            ...
            'requested_by_user' => $asset->isRequestedBy($user),
        ];
