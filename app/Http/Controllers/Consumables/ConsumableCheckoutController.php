...
class ConsumableCheckoutController extends Controller
{
     ...
     * @param int $id
     */
    public function create($id, $requestId = null) : View | RedirectResponse
    {
        ...
        if ($consumable = Consumable::find($id)) {
            ...
            if ($consumable->category) {
                ...
                return view('consumables/checkout', compact('consumable', 'requestId', 'checkoutRequest'));
            }
     ...
     */
    public function store(Request $request, $consumableId, $requestId = null)
    {
        ...
        {
            $checkoutRequest = CheckoutRequest::findOrFail($requestId);
            if ($request->input('assigned_to') != $checkoutRequest->user_id)
        ...
        if ($checkoutRequest->quantity != $quantity)
        {
            return redirect()->route('requestable-assets')->withFragment('consumables')->with('error', trans("The user did not select this quantity for this consumable. User selected $checkoutRequest->quantity" . '.'));
        }
        ...
        if ($requestId)
        {
            ...
            $requestName = $checkoutRequest->consumable->name;
            $user = User::find($assigned_to);
            $user->notify(new CheckoutRequestApprovedNotification($requestType, $requestName));
        }

        // Check if the user exists
        if (is_null($user = User::find($assigned_to))) {
            // Redirect to the consumable management page with error
            return redirect()->back()->with('error', trans('admin/consumables/message.checkout.user_does_not_exist'))->withInput();
        }
        ...
        if ($requestId) {
            return redirect()->route('assets.requested')->withFragment('consumables')->with('success', 'Asset has been successfully checked out.');
        }
