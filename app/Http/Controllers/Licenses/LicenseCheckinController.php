...
class LicenseCheckinController extends Controller
{
    ...
    public function store(Request $request, $seatId = null, $backTo = null)
    {
        ...
        // Find the checkout request that corresponds to the item being returned. Mark checkedin_at as now.
        $checkoutRequest = CheckoutRequest::where('id', $licenseSeat->checkout_request_id) // Ensure `checkout_request_id` matches
            ->first();
        ...
        if ($checkoutRequest)
        {
            $checkoutRequest->checkedin_at = Carbon::now();
