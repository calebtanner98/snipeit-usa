...
class LicenseCheckoutController extends Controller
{
    ...
    public function create($id, $requestId = null)
    {
        if ($license = License::find($id)) {
            ...
            if ($license->category) {
                ...
                return view('licenses/checkout', compact('license', 'requestId', 'checkoutRequest'));
            }
     ...
     * @throws \Illuminate\Auth\Access\AuthorizationException
     */
    public function store(LicenseCheckoutRequest $request, $licenseId, $seatId = null, $requestId = null)
    {
    ...
    // This one is used for checking out 
    public function storeCheckoutRequest(LicenseCheckoutRequest $request, $licenseId, $requestId = null)
    {
        if (! $license = License::find($licenseId)) {
            return redirect()->route('licenses.index')->with('error', trans('admin/licenses/message.not_found'));
        }
        ...
        // If request_id is provided, fetch the checkout request - check to make sure user selected is the user that requested
        $checkoutRequest = null;
        if ($requestId) 
        {
            $checkoutRequest = CheckoutRequest::findOrFail($requestId);
            $correctUser = $request->assigned_to == $checkoutRequest->user_id;
            if (!$correctUser)
            {
                return redirect()->back()->withFragment('licenses')->with('error', trans('The user selected did not request this license. Please select the user that requested the license.'));
            }
        }
        ...
        if ($correctUser)
        {
            ...
            $licenseSeat->checkout_request_id = $requestId;
            $licenseSeat->save();

            $requestType = 'License';
            $requestName = $checkoutRequest->license->name;

            $checkoutTarget->notify(new CheckoutRequestApprovedNotification($requestType, $requestName));
            
        }

        if ($requestId && $checkoutTarget )
        {
            return redirect()->route('assets.requested')->withFragment('licenses')->with('success', trans('admin/licenses/message.checkout.success'));
        } else {
            return redirect()->to(Helper::getRedirectOption($request, $license->id, 'Licenses'))->with('success', trans('admin/licenses/message.checkout.success'));
        }
        ...
        if ($requestId)
        {
            return redirect()->route('assets.requested')->with('error', trans('Something went wrong handling this checkout.'));
