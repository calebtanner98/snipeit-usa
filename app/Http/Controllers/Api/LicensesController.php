...
class LicensesController extends Controller
{
    ...
    // Added by USA team to allow users to view requestable licenses based off permission set

    public function requestableLicenses(Request $request) : JsonResponse | array
    {
        // Check if the user has permission to view requestable licenses
        $this->authorize('viewRequestable', License::class);
    
        $licenses = License::select('licenses.*')
            ->where('requestable', true);
    
        if ($request->filled('search')) {
            $licenses->where('name', 'like', '%' . $request->input('search') . '%');
        }
    
        $order = $request->input('order') === 'asc' ? 'asc' : 'desc';
        $column_sort = $request->input('sort') ?? 'licenses.created_at';
        ...
        return (new LicensesTransformer)->transformRequestedLicenses($licenses, $total);
    }
