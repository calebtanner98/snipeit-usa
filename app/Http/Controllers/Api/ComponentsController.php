...
class ComponentsController extends Controller
{
    ...
    */
    ...
    public function requestableComponents(Request $request): JsonResponse | array
    {

        $this->authorize('viewRequestable', Component::class);

        $components = Component::select('components.*')
            ->where('requestable', true);

        if ($request->filled('search')) {
            $components->where('name', 'like', '%' . $request->input('search') . '%');
        }

        $order = $request->input('order') === 'asc' ? 'asc' : 'desc';
        $column_sort = $request->input('sort') ?? 'components.created_at';
        $components->orderBy($column_sort, $order);
        ...
        return (new ComponentsTransformer)->transformRequestedComponent($components, $total);
    }
