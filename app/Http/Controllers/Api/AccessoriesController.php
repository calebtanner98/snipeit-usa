...
class AccessoriesController extends Controller
{
    ...
    */
    ...
    // Added by USA team to allow users to view requestable accessories based off permission set

    public function requestableAccessories(Request $request) : JsonResponse | array
    {
        $this->authorize('viewRequestable', Accessory::class);
    
        $accessories = Accessory::select('accessories.*')
            ->where('requestable', true);
    
        if ($request->filled('search')) {
            $accessories->where('name', 'like', '%' . $request->input('search') . '%');
        }
    
        $order = $request->input('order') === 'asc' ? 'asc' : 'desc';
        $column_sort = $request->input('sort') ?? 'accessories.created_at';
        $accessories->orderBy($column_sort, $order);
        ...
        return (new AccessoriesTransformer)->transformRequestedAccessory($accessories, $total);
    }
}