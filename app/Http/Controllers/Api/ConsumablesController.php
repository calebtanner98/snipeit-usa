...
class ConsumablesController extends Controller
{
    ...
    */
    ...
    public function requestableConsumables(Request $request): JsonResponse | array
    {

        $this->authorize('viewRequestable', Consumable::class);

        $consumables = Consumable::select('consumables.*')
            ->where('requestable', true);

        if ($request->filled('search')) {
            $consumables->where('name', 'like', '%' . $request->input('search') . '%');
        }

        $order = $request->input('order') === 'asc' ? 'asc' : 'desc';
        $column_sort = $request->input('sort') ?? 'consumables.created_at';
        $consumables->orderBy($column_sort, $order);
        ...
        // Use the transformer to return the formatted data
        return (new ConsumablesTransformer)->transformRequestedConsumables($consumables, $total);
    }
}