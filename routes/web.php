...
Route::group(['middleware' => 'auth'], function () {
    ...
    */
    ...
    Route::get('/user-portal', function () { return view('user-portal.index'); }); // Basic Route
    /*
     * Locations
     */
...
Route::middleware('auth')->group(function() {
    ...
    Route::post('/account/request-license/{license}', 
        [LicenseRequestController::class, 'requestLicense'])
         ->name('license.request');

    Route::post('/account/cancel-license-request/{licenseId}', 
        [LicenseRequestController::class, 'cancelLicenseRequest'])
         ->name('license.cancel');

    Route::post('/account/cancel-license-request/{licenseId}/{userId}', 
        [LicenseRequestController::class, 'postCancelLicenseRequest'])
         ->name('license.getCancel');

    Route::post('/account/license-wait-request/{licenseId}', 
        [LicenseRequestController::class, 'waitlistLicenseRequest'])
        ->name('license.waitlist');

    // Accessories

    Route::post('/account/request-accessory/{accessory}', 
        [AccessoryRequestController::class, 'requestAccessory'])
        ->name('accessory.request');

    Route::post('/account/cancel-accessory-request/{accessory_id}', 
        [AccessoryRequestController::class, 'cancelAccessoryRequest'])
        ->name('accessory.cancel');

    Route::post('/account/accessory-wait-request/{accessory_id}', 
        [AccessoryRequestController::class, 'waitlistAccessoryRequest'])
        ->name('accessory.waitlist');

    // Components

    Route::post('/account/request-component/{consumable}', 
        [ComponentRequestController::class, 'requestComponent'])
        ->name('consumable.request');

    Route::post('/account/cancel-component-request/{componend_id}', 
        [ComponentRequestController::class, 'cancelComponentRequest'])
        ->name('component.cancel');

    Route::post('/account/component-wait-request/{accessory_id}', 
        [ComponentRequestController::class, 'waitlistComponentRequest'])
        ->name('component.waitlist');

    // Consumables

    Route::post('/account/request-consumable/{component}', 
        [ConsumableRequestController::class, 'requestConsumable'])
        ->name('consumable.request');
    
    Route::post('/account/cancel-consumable-request/{consumable_id}',
        [ConsumableRequestController::class, 'cancelConsumableRequest'])
        ->name('consumable.cancel');
    
    Route::post('/account/consumable-wait-request/{consumable_id}', 
        [ConsumableRequestController::class, 'waitlistConsumableRequest'])
        ->name('consumable.waitlist');


    // Asset Waitlist

    Route::post('/account/asset-wait-request/{licenseId}', 
        [AssetRequestController::class, 'waitlistAssetRequest'])
        ->name('asset.waitlist');

    Route::post('/account/cancel-waitlist-request/{waitlistId}',
        [WaitListController::class, 'cancelWaitlistRequest'])
        ->name('waitlist.cancel');

    Route::post('/account/cancel-waitlist-admin/{waitlistId}', 
        [WaitListController::class, 'adminCancelWaitlistRequest'])
        ->name('waitlist.cancel');

    Route::post('/account/close-waitlist-admin/{waitlistId}', 
        [WaitListController::class, 'closeWaitlistRequest'])
        ->name('waitlist.cancel');

    Route::post('/account/deny-waitlist-admin/{waitlistId}', 
        [WaitListController::class, 'denyWaitlistRequest'])
        ->name('waitlist.deny');
});
...
// /*
//  |--------------------------------------------------------------------------
//  | ECOMMERCE UI - TEMPLATE - DID NOT COMPLETE
//  |--------------------------------------------------------------------------
// */

// Route::get('/portal', [UserPortalController::class, 'index'])->name('testing.index');
// Route::get('/portal/components', [UserPortalController::class, 'components'])->name('testing.components');
// Route::get('/portal/accessories', [UserPortalController::class, 'accessories'])->name('testing.accessories');
// Route::get('/portal/licenses', [UserPortalController::class, 'licenses'])->name('testing.licenses');
// Route::get('/portal/assets', [UserPortalController::class, 'assets'])->name('testing.assets');