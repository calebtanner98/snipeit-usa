...
@section('title')
    @if($checkoutRequest)
        Checkout Asset to {{$checkoutRequest->user->first_name}} {{$checkoutRequest->user->last_name}}
    @else
        ...
        <div class="col-md-7">
            <div class="box box-default">
                <form class="form-horizontal" method="post"
                    action="{{ route('hardware.checkout.store', ['assetId' => $asset->id, 'request_id' => $checkoutRequest ? $checkoutRequest->id : null]) }}"
                    autocomplete="off">
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group {{ $errors->has('status_id') ? 'error' : '' }}">
                            ...
                            <div class="col-md-7 required">
                                {{ Form::select('status_id', $statusLabel_list, $asset->status_id, ['class' => 'select2', 'style' => 'width:100%', '', 'aria-label' => 'status_id']) }}
                                {!! $errors->first(
                                    'status_id',
                                    '<span class="alert-msg" aria-hidden="true"><i class="fas fa-times" aria-hidden="true"></i> :message</span>',
                                ) !!}
                    ...
                    <x-redirect_submit_options index_route="hardware.index" :button_label="trans('general.checkout')" :disabled_select="!$asset->model"
                        :options="[
